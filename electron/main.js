...
function contentType(filePath) {
  const extension = path.extname(filePath).lowercase()
  const types = {
    '.css': 'text/css',
    '.html': 'text/html',
    '.js': 'text/javascript',
...
function createWindow() {
  ...
  const splash = '<!DOCTYPE html><html><body style="margin:0;background:#151515;color:#f4e8d0;font-family:sans-serif;display:grid;place-items:center;height:100vh"><h1 style="font-size:28px">Iniciando Kiosko...</h1></body></html>'
  ...
  if (fs.existsSync(distPath)) {
    startBuiltAppServer(path.dirname(distPath))
      ...
      .catch((error) => {
        ...
        win.loadURL('data:text/html;charset=utf-8,' + encodeURIComponent('<h1 style="font-family:sans-serif;padding:32px">Kiosko no pudo iniciar</h1>'))
      })
  } else {
